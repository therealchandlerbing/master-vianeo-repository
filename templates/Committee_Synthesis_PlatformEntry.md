# [Project Name] - Vianeo Platform Entry

**Purpose:** Copy/paste guide for Vianeo platform submission

---

## Form Fields

**Title:** [Project Name]
**Date:** YYYY-MM-DD

---

## Scores (Select in Platform)

| Dimension | Score |
|-----------|:-----:|
| **Team** | X/5 |
| **Technology** | X/5 |
| **Management and Business** | X/5 |
| **Commercial** | X/5 |

---

## Comment (Copy/Paste Below)

**STRENGTHS:** [1-2 sentences with specific metrics]

**RISKS:** [1-2 sentences with quantified gaps]

**NEAR-TERM ACTIONS:** (1) [Action]. Owner: [Name]. Timeline: [X days]. (2) [Action]. Owner: [Name]. Timeline: [X days]. (3) [Action]. Owner: [Name]. Timeline: [X days].

**EVIDENCE GAPS:** [1-2 sentences on missing validation]

**RECOMMENDATION:** [PROCEED/CONDITIONAL PROCEED/PAUSE/DO NOT PROCEED]. [Brief justification].

---

## Score Rationale (Reference)

### Team: X/5
[2-3 sentence justification with evidence. Include team size, expertise areas, commitment level, and any gaps.]

### Technology: X/5
[2-3 sentence justification with evidence. Include prototype status, validation achieved, IP position, and differentiation.]

### Management and Business: X/5
[2-3 sentence justification with evidence. Include funding status, business model validation, ecosystem engagement, and financial projections.]

### Commercial: X/5
[2-3 sentence justification with evidence. Include customer interview count, pricing validation, go-to-market readiness, and traction.]

---

## Quick Reference

| Vianeo Dimension | Score | Committee Mapping |
|------------------|:-----:|-------------------|
| Legitimacy | X.X/5 | -> Team, Technology |
| Desirability | X.X/5 | -> Commercial |
| Acceptability | X.X/5 | -> Management and Business |
| Feasibility | X.X/5 | -> Team, Technology |
| Viability | X.X/5 | -> Management and Business, Commercial |

---

## Overall Metrics

| Metric | Score |
|--------|:-----:|
| Market Maturity Score | X.X/5.0 |
| Overall Vianeo Score | X.X/5.0 |
| Committee Average | X.X/5.0 |

---

## Character Count Check

Before submitting, verify character limits:

| Field | Limit | Your Count |
|-------|-------|------------|
| Title | 100 | [ ] |
| Comment | 2000 | [ ] |
| Per-dimension rationale | 500 | [ ] |

---

## Submission Checklist

- [ ] All 4 scores selected in platform
- [ ] Comment copied and pasted
- [ ] Character limits verified
- [ ] No em dashes in text
- [ ] All actions have owners and timelines
- [ ] Recommendation included

---

*Platform entry prepared: YYYY-MM-DD*
*Framework: Vianeo Business Model Evaluation System*
